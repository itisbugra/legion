defmodule Legion.Identity.Auth.ConcreteTest do
  ...
  describe "generate_passphrase/4" do
    ...
    test "matching username/password combination returns passkey", %{password_hash: password_hash, pair: pair, ip_addr: ip_addr} do
      assert match? {:ok, :require, _}, generate_passphrase(pair.username, password_hash, ip_addr)
    end

    test "if user does not exist says no user", %{password_hash: password_hash, ip_addr: ip_addr} do
      assert {:error, :no_user_verify} == generate_passphrase("_failing_some_username", password_hash, ip_addr)
    end
    ...
    test "errors if user already has maximum amount of passphrases exceed", %{pair: pair, password_hash: password_hash, ip_addr: ip_addr} do
      ...
      assert {:error, :maximum_passphrases_exceeded} == generate_passphrase(pair.username, password_hash, ip_addr)
    end

    @tag :regression
    test "outdated passphrases do not count in allowance limit", %{pair: pair, password_hash: password_hash, ip_addr: ip_addr} do
      invalidation_moment = add(utc_now(), -(@passphrase_lifetime + 1), :second)

      _passphrases = insert_list(@maximum_allowed_passphrases, :passphrase, user: pair.user, inserted_at: invalidation_moment)

      assert match? {:ok, :require, _}, generate_passphrase(pair.username, password_hash, ip_addr)
    end

    @tag :regression
    test "invalidated passphrases do not count in allowance limit", %{pair: pair, password_hash: password_hash, ip_addr: ip_addr} do
