defmodule Legion.Identity.Information.Telephony.Factory do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      alias Legion.Identity.Telephony.PhoneNumber.{PrioritizationTrait, SafetyTrait, NeglectionTrait}
      ...
      def phone_number_prioritization_trait_factory do
        %PrioritizationTrait{phone_number: build(:phone_number),
                             authority: build(:passphrase)}
      end

      def phone_number_safety_trait_factory do
        %SafetyTrait{phone_number: build(:phone_number),
                     authority: build(:passphrase),
                     valid_for: @six_months}
      end

      def phone_number_neglection_trait_factory do
        %NeglectionTrait{phone_number: build(:phone_number),
                         authority: build(:passphrase)}
      end
    end
  end
