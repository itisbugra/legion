defmodule Legion.Identity.Auth.Abstract.Token do
  ...
  @jwk %{"kty" => "oct", "k" => Keyword.fetch!(@env, :secret_key_base)}
  ...
  @spec issue_token(User.id(), Passphrase.id(), Keyword.t()) ::
          ...
          | {:error, :invalid}
          | {:error, :timed_out}
  def issue_token(user_id, passphrase_id, opts \\ [])
      when is_integer(user_id) and is_integer(passphrase_id) do
    env = Application.get_env(:legion, Legion.Identity.Auth.Concrete.JOSE)
    ...
    {:ok, %__MODULE__{header_value: token, expires_after: expires_after, jwk: @jwk, jws: @jws}}
  end
